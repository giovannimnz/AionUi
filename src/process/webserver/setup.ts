...
export function setupBasicMiddleware(app: Express): void {
  ...
  app.use(
    csrf(
      ...
      ['/login', '/api/auth/qr-login', '/api/upload', '/api/user-settings', '/channels/wecom/webhook'], // Excluded: login form, QR login, file upload (API token), user settings (authenticated), WeCom server callback (signed by WeCom)
